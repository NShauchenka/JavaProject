...
public class InputFacultyFormController implements ActionListener {
    ...
    public InputFacultyFormController(FacultyDC facultyDC,
            CourseDC offeredCourseDC, String fileLocation) {
    ...
    private void saveData() {
        ...
        String staffname = form.getNameField().getText();
        // Retrieve data from all text fields and store directly into object
        // Flags errors on the required fields
        
         if (isEmpty(staffname)) {
                throw new InvalidDataException("Please, enter the name");
            }
         newFaculty.setName(staffname);
        ...
        // and thenconvert them to integers
        String year = form.getDateOfHireYear().getValue().toString();
        String day = form.getDateOfHireDay().getValue().toString();
        String month = form.getDateOfHireMonth().getValue().toString();
        
        if (isEmpty(year) || isEmpty(day) || isEmpty(month)) {
             throw new InvalidDataException("Please, set date of hire!");
        }
        
        Integer dohYear = Integer.parseInt(year);
        Integer dohDay = Integer.parseInt(day);
        // Let's use a different month mapper for date of hire and convert the 
        // string month to a GregorianCalendar class month
        Integer dohMonth = this.getMonthFromMonthArray(month);
        ...
        this.facultyDC.getListOfFaculties().add(newFaculty);
        
        FacultyIO.writeXMLFile(fileLocation, facultyDC);

        FacultyJSONiO.writeJSONFile(fileLocation, facultyDC);
